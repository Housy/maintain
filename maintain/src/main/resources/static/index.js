...
layui.extend({
  ...
  ,entryPage = function(fn){
    ...
    //独立页面
    if(isIndPage || pathURL === '/userCenter/login' || pathURL === '/userCenter/register'){ //此处单独判断登入页，是为了兼容旧版（即未在 config.js 配置 indPage 的情况）
      ...
      if(setter.interceptor){
    	  ...
    	  if(code == -1) { //未登录
    		  return location.hash = '/userCenter/login/redirect='+ encodeURIComponent(pathURL); //跳转到登入页
    	  }
      }
  ...
  //点击返回按钮
  $(document).on('click', '#LAY-BACK', function(){
	  history.back();
  })
